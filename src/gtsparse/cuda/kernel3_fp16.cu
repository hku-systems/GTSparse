...
__device__ __forceinline__ void mma_m16n8k16(
    ...
    const unsigned* a_fragment = reinterpret_cast<const unsigned*>(a_fragment_half);
    const unsigned* b_fragment = reinterpret_cast<const unsigned*>(b_fragment_half);
...
__device__ __forceinline__ void kernel3_fp16_tile(
    const Kernel3FP16Params& p,
    int template_id,
    int out_row_base,
    int input_row_offset,
    ...
    for (int k_loop = 0; k_loop < total_k_loops; ++k_loop) {
        ...
        for (int k_half = 0; k_half < 2; ++k_half) {
            ...
            for (int m_tile = 0; m_tile < kMTiles; ++m_tile) {
                #pragma unroll
                for (int n_pair = 0; n_pair < kNPairs; ++n_pair) {
                    mma_m16n8k16(
...
__global__ void __launch_bounds__(kThreads, 4) kernel3_fp16_kernel(Kernel3FP16Params p) {
...
torch::Tensor kernel3_fp16_forward(
    ...
    int64_t n_out) {
#ifndef NDEBUG
    TORCH_CHECK(features.scalar_type() == at::kHalf, "features must be float16");
    TORCH_CHECK(logical_weight.scalar_type() == at::kHalf, "weight must be float16");
    ...
    kernel3_fp16_kernel<<<grid, kThreads, 0, at::cuda::getCurrentCUDAStream()>>>(p);
    return output;
}
