...
__device__ __forceinline__ int template_boundary_bump(int template_id, int slot) {
    ...
    if (template_id == kTemplateSkip1Hole0) {
        return slot == 1 || slot == 3 ? 1 : 0;
    }
    if (template_id == kTemplateSkip1Hole1) {
        return slot == 0 || slot == 5 ? 1 : 0;
    }
...
__device__ __forceinline__ void kernel9_fp32_tile(
    const Kernel9FP32Params& p,
    int template_id,
    int out_row_base,
    int input_row_offset,
    ...
    const float* slot_weight_ptr =
        p.weight + template_initial_offset(template_id) * logical_stride
        + (tid / 16) * cout + bn_base + ((tid * 4) % 64);
    ...
    for (int k_loop = 0; k_loop < total_k_loops; ++k_loop) {
        const float* weight_ci_ptr = slot_weight_ptr + ci_offset * cout;
        __syncthreads();
        ...
        const float* weight_ptr = weight_ci_ptr;
        #pragma unroll
    ...
    for (int index = 0; index < 64; ++index) {
        ...
        if (out_row >= 0 && col < cout) p.output[out_row * cout + col] = accum[index];
    }
}

__global__ void __launch_bounds__(kThreads) kernel9_fp32_kernel(Kernel9FP32Params p) {
...
torch::Tensor kernel9_fp32_forward(
    ...
    kernel9_fp32_kernel<<<grid, kThreads, 0, at::cuda::getCurrentCUDAStream()>>>(p);
    return output;
}
