...
class GeometricTemplateKernel3Conv3d(nn.Module):
    def __init__(
        ...
        *,
        ...
        if self.kernel_size != (3, 1, 1):
            raise ValueError("kernel-3 specialization requires kernel_size=(3, 1, 1)")
        if self.dilation[1:] != (1, 1):
            raise ValueError("kernel-3 specialization requires unit H/W dilation")
        if self.in_channels % 32 != 0 or self.out_channels % 64 != 0:
            raise ValueError("kernel-3 setting requires Cin divisible by 32 and Cout divisible by 64")
        self.max_bm = int(max_bm)
        self.weight = nn.Parameter(torch.empty(3, self.in_channels, self.out_channels))
    ...
    def reset_parameters(self) -> None:
        ...
        bound = 1.0 / math.sqrt(float(self.in_channels * 3))
    ...
    def _runtime_weight(self) -> torch.Tensor:
        if self.runtime_weight_cache is None:
            self.runtime_weight_cache = self.weight.contiguous().view(3 * self.in_channels, self.out_channels)
    ...
    def build_runtime(self, x: GTSparseSparseConvTensor):
        ...
        tensors = _C.gtsparse_kernel3_build_full_runtime_from_coords(
            x.indices,
            *out_spatial,
            *self.stride,
            *self.padding,
        ...
        return Kernel3Runtime(*tensors), out_spatial
